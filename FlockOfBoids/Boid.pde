class Boid {
  ...
  void flock(ArrayList<Boid> boids, ArrayList<Predator> predators) {
    ...
    Vector repulse;
    //survive
    Vector posPred = new Vector(0, 0, 0);
    int predatorCount = 0;
    for (int i = 0; i < boids.size(); i++) {
      ...
      for (int x=0;x<predators.size();++x){
       Predator predator = predators.get(x);
       float predatorDistance = Vector.distance(position, predator.position);
       if (predatorDistance > 0 && predatorDistance <= neighborhoodRadius) {
        posPred.add(boid.position);
        predatorCount++;
      }
    ...
    if (predatorCount >0){
     posPred.divide((float)predatorCount);
     posPred.subtract(position);
     posPred.limit(maxSteerForce);
    }
    ...
    acceleration.add(Vector.multiply(posPred, 10));
  }
